...
def MSE(target, predicted):
    ...
    tar1 = target[:,0,:,:].cpu().detach().numpy()
    out1 = predicted[:,0,:,:].cpu().detach().numpy()
...
def meanIOU(target, predicted, th = 0.05):
    ...
    for i in range(target.shape[0]):
        ...
        target_arr = target[i, :, :, :].clone().detach().cpu().numpy()[0]
        target_arr[target_arr > th] = 1
        target_arr[target_arr < th] = 0
        #predicted_arr = predicted[i, :, :, :].clone().detach().cpu().numpy()[1]
        predicted_arr = predicted[i, :, :, :].clone().detach().cpu().numpy()[0]
        predicted_arr[predicted_arr > th] = 1
        predicted_arr[predicted_arr < th] = 0
...
def pixelAcc(target, predicted):
    ...
    for i in range(target.shape[0]):
        #target_arr = target[i, :, :, :].clone().detach().cpu().numpy()[1]
        target_arr = target[i, :, :, :].clone().detach().cpu().numpy()[0]
        target_arr[target_arr > 0.1] = 1
        target_arr[target_arr < 0.1] = 0
        #predicted_arr = predicted[i, :, :, :].clone().detach().cpu().numpy()[1]
        predicted_arr = predicted[i, :, :, :].clone().detach().cpu().numpy()[0]
        predicted_arr[predicted_arr > 0.1] = 1
        predicted_arr[predicted_arr < 0.1] = 0
